...
public class RCC_CarControllerV3 : RCC_Core {
    ...
    private void Update() {
        ...
        float rearSidewaysSlip = Mathf.Abs(RearLeftWheelCollider.wheelSlipAmountSideways) + Mathf.Abs(RearRightWheelCollider.wheelSlipAmountSideways);
        rearSidewaysSlip /= 2f;

        if (rearSidewaysSlip > .25f)
            driftingNow = true;
        else
            driftingNow = false;
    ...
    private void ESPCheck() {
        ...
        frontSlip = Mathf.Abs(FrontLeftWheelCollider.wheelSlipAmountSideways) + Mathf.Abs(FrontRightWheelCollider.wheelSlipAmountSideways);
        rearSlip = Mathf.Abs(RearLeftWheelCollider.wheelSlipAmountSideways) + Mathf.Abs(RearRightWheelCollider.wheelSlipAmountSideways);

        if (frontSlip >= ESPThreshold)
            underSteering = true;
        else
            underSteering = false;

        if (rearSlip >= ESPThreshold)
            overSteering = true;
        else
            overSteering = false;
